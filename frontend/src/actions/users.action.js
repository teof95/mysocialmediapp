...
export const getUserById = (user_id) => async (dispatch) => {
  try {
    const res = await axios.get(
      `http://hatebooksocial.herokuapp.com/api/users/get_user_by_id/${user_id}`
    );
...
export const getUserPosts = () => async (dispatch) => {
  ...
  export const getUserPostsById = (user_id) => async (dispatch) => {
    try {
      const res = await axios.get(
        `http://hatebooksocial.herokuapp.com/api/posts/user_posts/${user_id}`
      );
  ...
  export const getUsers = () => async (dispatch) => {
    try {
      const res = await axios.get(`http://hatebooksocial.herokuapp.com/api/users/users`);
  ...
  export const searchByUsername = (userNameFromSearch) => async (dispatch) => {
    try {
      ...
      const response = await axios.put(
        `http://hatebooksocial.herokuapp.com/api/users/search_by_username`,
        body,
        config
      );
      dispatch({
